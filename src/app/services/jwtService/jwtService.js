...
class jwtService extends FuseUtils.EventEmitter {
        ...
        return new Promise((resolve, reject) => {
            api.post('/auth/login', {
                ...
                if (response.data.decodedToken) {
                    // if (response.data.decodedToken && response.data.decodedToken.role === 'admin' || 'superAdmin') {
                    this.setSession(response.data.access_token);
    ...
    isSuperAdmin = () => {
        var access_token = localStorage.jwt_access_token
        if (!access_token) {
            return false;
        }
        const decoded = jwtDecode(access_token);
        ...
        if (decoded.exp < currentTime) {
            ...
            return decoded['role'] == "superAdmin" ? true : false;
        }
    };
